...
#' Cross join
#'
#' Uses \code{\link[data.table]{data.table}} to efficiently perform a cross
#' join between two data frames; that is, the Cartesian product of the rows
...
#'
#' @return A \code{\link[data.table]{data.table}} containing the Cartesian
#' product of the rows from \code{x} and \code{y}.
#'
#' @keywords internal
#'
#' @import data.table
crossJoin <- function(x, y) {
  ...
  Reduce(
    f = function(x, y) cbind(x, y[rep(1:.N, each = nrow(x))]),
    x = list(x, y)
  )
}
...
#'
#' @import data.table
getParDepRegInMem <- function(object, pred.var, pred.grid, train, ...) {

  # Perform a cross join between feature grid and trainin data
  train[, names(pred.grid)] <- NULL  # remove features of interest
  ice_data <- crossJoin(train, y = pred.grid)  # Cartesian product

  # Convert ice_data to the same class as train if train is not a data frame
  if (inherits(train, "matrix")) {
    ice_data <- data.matrix(ice_data)
  }
  if (inherits(train, "dgCMatrix")) {
    ice_data <- methods::as(data.matrix(ice_data), "dgCMatrix")
  }

  # Score the data, remove extra columns, and aggregate
  ice_data[["yhat"]] <- getIcePredReg(object, newdata = ice_data, ...)
  ice_data <- ice_data[, c(names(pred.grid), "yhat"), with = FALSE]
  ice_data[, lapply(.SD, FUN = mean), by = names(pred.grid), .SDcols = "yhat"]
