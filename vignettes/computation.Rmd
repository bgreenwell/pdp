...
Next, we use the multivariate adaptive regression splines (MARS) algorithm introduced in @friedman-1991-mars to model ozone concentration as a nonlinear function of the eight meteorological variables plus day of the year; we allow for up to three-way interactions.
```{r ozone-mars-02}
library(earth)  # for earth() function (i.e., MARS algorithm)
ozone.mars <- earth(ozone ~ ., data = ozone, degree = 3)
summary(ozone.mars)
```
The MARS model produced a generalized $R^2$ of `r ozone.mars$r.squared`, similar to what was reported in @breiman-1985-estimating. A single three-way interaction was found involving the predictors
