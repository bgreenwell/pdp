...
In the code chunk below, we use the **ranger** package [@ranger-pkg] to fit a random forest to the built-in iris data. Note that in order to use the **vip** [@pkg-vip] and **pdp** [@pdp-pkg] packages for classification models, we need to be able to compute the predicted class probabilities. Therefore, we must specify `probability = TRUE` in the call to `ranger()`.
